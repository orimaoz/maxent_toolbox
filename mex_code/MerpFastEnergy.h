...
public:
	...
	MerpFastEnergy(double* in_W, double * in_lambda, uint32_t ncells, uint32_t nfactors, double cutoff, double bStochasticSlope = 0);

	// Constructor - construct it from a random projection and a set of thresholds
	MerpFastEnergy(double* in_W, double * in_lambda, uint32_t ncells, uint32_t nfactors, double * in_thresholds, double bStochasticSlope = 0);
