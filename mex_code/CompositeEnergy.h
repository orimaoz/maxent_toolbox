// Class that implements a generic composite energy function built out of several other energy functions
...
class CompositeEnergy : public EnergyFunction
{
public:

		
	// Constructor - constructs an empty composite
	CompositeEnergy(uint32_t ncells);


	// destructor - removes all internally kept energy function (the destructor deletes them
	~CompositeEnergy();

	// makes it handle an additional energy function
	void addEnergyFunction(EnergyFunction * energy);
...
private:
	...
	// internal energy functions that we keep and delegate to
	std::vector<EnergyFunction *> m_internalEnergies;
