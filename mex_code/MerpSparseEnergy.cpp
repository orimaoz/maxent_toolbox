...
MerpSparseEnergy::MerpSparseEnergy(float* in_W, double * in_lambda, uint32_t ncells, uint32_t nfactors, float * in_thresholds, double bStochasticSlope)
	: m_ndims(ncells), m_nfactors(nfactors), m_logz(0), m_bProposed(false)
{
	...
	m_W_val = (float**)malloc_aligned(sizeof(double*)*m_ndims);	// values of each column of W
	...
	for (uint32_t idxCol = 0; idxCol < ncells; idxCol++)
	{
		...
		m_W_val[idxCol] = (float*)malloc_aligned(sizeof(float)*m_nfactors);	// values of current column
		m_W_idx[idxCol] = (MKL_INT*)malloc_aligned(sizeof(MKL_INT)*m_nfactors);	// indices of current column

		float * p_sparse_W_val = m_W_val[idxCol];
...
#pragma vector aligned
	for (uint32_t i = 0; i < m_ndims; i++)
	{
		...
		if (x[i])
		{
			...
			cblas_saxpyi(m_W_nz[i], 1, m_W_val[i], m_W_idx[i], m_y);
...
double MerpSparseEnergy::applyThreshold(float * y)
{
...
double MerpSparseEnergy::accept(uint32_t bAccept)
{
	if (bAccept)
	{
		...
		{
			// bit changing 1->0
			cblas_saxpyi(m_W_nz[m_proposed_bit], -1, m_W_val[m_proposed_bit], m_W_idx[m_proposed_bit], m_y);
		}
		else
		{
			// bit changing 0->1
