...
if exist(reentry_filename, 'file') == 2
    ...
    stderr=nan(1,nfactors);  % square errors for each factor
    ...
    step_scaling = maxent.getStepScaling(model);
...
while i < num_steps
    ...
    params.burnin = 0;
    x = mexGibbsSampler(model,nsamples,x0,params);
    ...
    if curr_iteration < GIS_INITIAL_STEPS        
        ...
        model = maxent.setFactors(model,maxent.getFactors(model) - delta);
        
        % save momentum for accelerated gradient descent (for when we switch)
        y = maxent.getFactors(model)+ curr_step_size .* factor_gradient;
    else        
        ...
        if (use_acceleration)
            ...
            model = maxent.setFactors(model,maxent.getFactors(model) + curr_step_size .* factor_gradient);
